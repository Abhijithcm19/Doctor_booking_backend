...
export async function getSingleDoctor(req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .send({ success: true, msg: "doctor Found...!", data: doctor });
  } catch (error) {
...
export const getDoctorsByService = async (req, res) => {
  try {
    ...
    const doctors = await DoctorModel.aggregate([
      {
        $match: { specialization: new mongoose.Types.ObjectId(serviceId) },
      },
      {
        $lookup: {
          from: "services",
    ...
    if (!doctors || doctors.length === 0) {
      return res
        .status(404)
        .json({ message: "No doctors found for this service" });
    }
